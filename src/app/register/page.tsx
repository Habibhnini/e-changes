...
export default function ChatPage() {
  ...
  // Function to authenticate with Mercure - EXACT same as in MercureTest
  async function authenticateMercure() {
    try {
      console.log("Authenticating with Mercure...");
  ...
  // Function to connect to Mercure via the proxy - based on the working MercureTest
  async function connectToMercure() {
    if (!selectedTransactionId) {
      console.log("No transaction ID, skipping Mercure connection");
      return;
    }
    ...
    try {
      // Use your local proxy with EXACTLY the same URL structure as the test
      ...
      eventSource.onopen = () => {
        ...
        setConnectionStatus("Connected to Mercure Hub");
      };
        ...
        try {
          ...
          if (data.type === "new_message" && data.message) {
            ...
            setMessages((prev) => {
              ...
              // Set isYou flag based on sender ID
              const newMessage = {
                ...data.message,
                sender: {
                  ...data.message.sender,
          ...
          // Handle conversation updates
            ...
            console.log("Conversation update received:", data.conversation);
      ...
      setConnectionStatus("Setup failed: " + (err as Error).message);
    }
  }
  ...
  useEffect(() => {
    if (selectedTransactionId && currentUser) {
      connectToMercure();
    }
  ...
  // Function to test the Mercure connection - copy of the test function
  const testMercure = async () => {
    try {
      console.log("Testing Mercure connection...");
      const response = await fetch("/api/messages/mercure/test");
      const data = await response.json();

      if (data.success) {
        console.log("Test message published successfully:", data);
      } else {
      ...
      alert("Error publishing test message: " + (err as Error).message);
    }
  };
  ...
  const handleSendMessage = useCallback(
    async (e: React.FormEvent) => {
      ...
      if (messageInput.trim() && selectedTransactionId) {
        ...
        console.log(
          `Sending message: "${messageContent}" to transaction ${selectedTransactionId}`
        );
        ...
        try {
          ...
          console.log(
            "Message sent successfully, server response:",
            sentMessage
          );
  ...
  const renderDebugInfo = () => (
    <div className="fixed top-0 right-0 bg-white p-2 border border-gray-300 rounded shadow-md text-xs m-2 z-50 max-w-xs">
      <div className="font-bold mb-1">Debug Info</div>
      <div>Connection: {connectionStatus}</div>
      <div>Transaction: {selectedTransactionId}</div>
      {lastEvent && (
        <div className="truncate text-gray-600 text-xs">
          Last event: {lastEvent.substring(0, 30)}...
        </div>
      )}
